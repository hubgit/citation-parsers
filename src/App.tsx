...
const parseInput = async (input: string) => {
  ...
  for (const path of parsers) {
    const {
      default: { name, url, parse },
    } = await import(`./parsers/${path}`)
